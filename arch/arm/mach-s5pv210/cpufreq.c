...
void liveoc_update(unsigned int oc_value, unsigned int oc_above, unsigned int oc_under)
{
	...
	if(s5pv210_freq_table[i].frequency >= oc_above)
		fclk = (original_fclk[index] * oc_value) / 100;
	else if(s5pv210_freq_table[i].frequency <= oc_under)
		fclk = (original_fclk[index] * oc_value) / 100;
	else
