...
Value* AROMA_INI_SET(const char* name, State* state, int argc, Expr* argv[]) {
  ...
  else if (strcmp(args[0],"compiled_by") == 0)   	snprintf(acfg()->compiled_by,63,args[1]);
...
byte aui_start(){
  ...
  if (error != 0 || error_count > 0) {
      vibrate(50);
      fprintf(apipe(),"ui_print\n");
      fprintf(apipe(),"ui_print SYNTAX ERROR! icssgs-config on line %d col %d\n",yyErrLine(),yyErrCol());
      fprintf(apipe(),"ui_print\n");
      usleep(200000);
      vibrate(50);
      return 0;
  ...
  if (result == NULL) {
    ...
    if (state.errmsg == NULL){
        fprintf(apipe(),"ui_print\n");
        fprintf(apipe(),"ui_print ICSSGS Installer Terminated...\n");
        fprintf(apipe(),"ui_print\n");
        res = 1;
    }
    else{
        vibrate(50);
        fprintf(apipe(),"ui_print\n");
        fprintf(apipe(),"ui_print FUNCTION ERROR! icssgs-config: %s\n",state.errmsg);
        fprintf(apipe(),"ui_print\n");
        usleep(200000);
        vibrate(50);
    }
