...
#set log
if [ -e buildinstaller.log ] ; then
	rm buildinstaller.log
	exec >> buildinstaller.log 2>&1
	set -x
else
	exec >> buildinstaller.log 2>&1
	set -x
...
#replace "/opt/toolchains/arm-2009q3/bin/arm-none-linux-gnueabi-gcc" with the correct path to "arm-none-linux-gnueabi-gcc"
...
mv bin/update-binary out/update-binary
